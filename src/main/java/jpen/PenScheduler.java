...
final class PenScheduler {
  ...
  /**
   * The SystemMouseFilter is used to filter system mouse movement to avoid conflict with movements
  ...
  private static class SystemMouseFilter {
    ...
    /**
     * @return {@code true} if the device is the system mouse and other device has already scheduled
     *     level events
     */
    boolean filterOut(PenDevice device) {
      if (device == penManager.emulationDevice) return false;
      if (penManager.isSystemMouseDevice(device)) {
        ...
        preferredKindTypeNumber = device.getKindTypeNumber();
        if (penManager.getSystemMouseProvider()
            != null) { // filter out level events when coming dragging in from another AWT
          // component.
          if (mayBeDraggingIn) return true;
        }
    ...
    void setLastLevelEvent(PLevelEvent lastLevelEvent) {
      if (lastLevelEvent.getDeviceId() == penManager.emulationDevice.getId()) return;
    ...
    if (lastScheduledState.levels.getValue(PLevel.Type.PRESSURE) > 0)
      scheduleLevelEvent(
          new PLevelEvent(
              ...
              new PLevel[] {new PLevel(PLevel.Type.PRESSURE, 0)}));
  }
  ...
  synchronized boolean scheduleLevelEvent(
    ...
    if (systemMouseFilter.filterOut(device)) return false;
    ...
    PenOwner penOwner =
        pen.penManager == null
            ? null
            : pen.penManager.penOwner; // pen.penManager can be null when running tests
    if (penOwner != null && levelsOnScreen)
      penOwner.getPenClip().evalLocationOnScreen(clipLocationOnScreen);
    for (PLevel level : levels) {
      if (device != getEmulationDevice()
          && pen.levelEmulator != null
          && pen.levelEmulator.onActivePolicy(
              lastScheduledState.getKind().getType().ordinal(), level.typeNumber)) continue;
      if (level.value.isNaN()) continue;
      if (level.isMovement()) {
        float levelValue = level.value;
        switch (level.getType()) {
          case X:
            if (levelsOnScreen) levelValue -= clipLocationOnScreen.x;
            scheduledLocation.x = levelValue;
            break;
          case Y:
            if (levelsOnScreen) levelValue -= clipLocationOnScreen.y;
        ...
        if (!firstScheduleAfterPause) {
          ...
          if (levelValue == lastLevelValue) continue;
          if (pen.penManager.isSystemMouseDevice(device)
              && Math.abs(levelValue - lastLevelValue) < 1) continue;
        }
        level.value = levelValue;
        scheduledMovement = true;
      } else {
        if (!firstScheduleAfterPause
            && level.value == lastScheduledState.getLevelValue(level.typeNumber)) continue;
        switch (level.getType()) {
          case PRESSURE:
    ...
    if (scheduledLevels.isEmpty()) return false;

    if (scheduledMovement) {
      if (penOwner != null
          && !penOwner.isDraggingOut()
          && !penOwner.getPenClip().contains(scheduledLocation)) return false;
    }
    ...
    scheduleLevelEvent(
        new PLevelEvent(
            device, deviceTime, scheduledLevels.toArray(new PLevel[scheduledLevels.size()])));
  ...
  private void schedule(PenEvent ev) {
    ...
    // System.out.println("*** scheduled event *** "+System.currentTimeMillis()+",
    // ev.time="+ev.time);
