...
public final class PenManager {
  ...
  private static synchronized void incrementInstanceCount() {
    if (singletonMode && instanceCount != 0)
      throw new IllegalStateException(
          "can't create more than one PenManager when in singleton mode");
    instanceCount++;
  }

  /** See {@link PenOwner#enforceSinglePenManager()}. */
  private static synchronized void setSingletonMode(boolean singletonMode) {
    if (singletonMode && instanceCount != 0)
      throw new IllegalStateException(
          "can't change singleton mode to true when another PenManager has already been created");
    PenManager.singletonMode = singletonMode;
  }

  public final Pen pen = new Pen(this);
  public final PenOwner penOwner;
  private final Set<PenProvider.Constructor> providerConstructors =
      Collections.synchronizedSet(new HashSet<PenProvider.Constructor>());
  private final Set<PenProvider.Constructor> providerConstructorsA =
      Collections.unmodifiableSet(providerConstructors);
  private final Map<Byte, PenDevice> deviceIdToDevice =
      new HashMap<Byte, PenDevice>(Byte.MAX_VALUE, 1f);
  private final Collection<PenDevice> devicesA =
      Collections.unmodifiableCollection(deviceIdToDevice.values());
  ...
  /**
   * Creates an {@code AwtPenOwner} and calls the {@link #PenManager(PenOwner)} constructor.
  ...
  public PenManager(PenOwner penOwner) {
    if (penOwner.enforceSinglePenManager()) setSingletonMode(true);
    ...
    synchronized (pen.scheduler) {
      ...
      penOwner.setPenManagerHandle(
          new PenOwner.PenManagerHandle() {
            // @Override
  ...
  /**
   * In some cases, constructing providers takes considerable time (wintab), so we add them in a new
   * thread.
   */
  private void addPenOwnerProviders() {
    Thread thread =
        new Thread("jpen-PenManager-addPenOwnerProviders") {
          @Override
          public void run() {
            synchronized (pen.scheduler) {
              for (PenProvider.Constructor penProviderConstructor :
                  PenManager.this.penOwner.getPenProviderConstructors())
  ...
  private synchronized void waitForProviderConstructorsInitialization() {
    while (!providerConstructorsInitialized) ObjectUtils.waitUninterrupted(this);
  }
  ...
  /**
   * Constructs and adds provider if {@link PenProvider.Constructor#constructable(PenManager)} is
   * true.
   *
  ...
  private PenProvider addProvider(PenProvider.Constructor providerConstructor) {
    if (providerConstructor.constructable(this)) {
      ...
      if (providerConstructor.construct(this)) {
        ...
        if (!getPaused())
          provider.penManagerPaused(
              false); // the provider is paused after construction, so it is not necessary to call
        // when true
  ...
  public void firePenDeviceAdded(PenProvider.Constructor constructor, PenDevice device) {
    ...
    if (systemMouseDevice == null && device instanceof MouseDevice) this.systemMouseDevice = device;
  ...
  private byte getNextDeviceId() {
    ...
    while (deviceIds.contains(Byte.valueOf(nextDeviceId))) nextDeviceId++;
    if (nextDeviceId < 0) throw new IllegalStateException();
    return nextDeviceId;
  }
  ...
  public void firePenDeviceRemoved(PenProvider.Constructor constructor, PenDevice device) {
    ...
    if (systemMouseDevice == device) this.systemMouseDevice = null;
  }
  ...
  void setPaused(boolean paused) {
    synchronized (pen.scheduler) {
      ...
      this.paused = paused;
      PenProvider.Constructor[] providerConstructorsArray =
          providerConstructors.toArray(
              new PenProvider.Constructor
                  [0]); // I don't want to wait for the providerConstructors initialization so I do
      // a copy.
      for (PenProvider.Constructor providerConstructor : providerConstructorsArray) {
        ...
        if (penProvider != null) penProvider.penManagerPaused(paused);
      }
    }
  }
  ...
  /**
   * Schedules button events. You must construct a new {@code PButton} each time you call this
   * method (do not reuse).
   *
  ...
  /**
   * Schedules scroll events. You must construct a new {@code PScroll} each time you call this
   * method (do not reuse).
   *
   * @param device source device
   * @param deviceTime time of event creation on device
  ...
  /**
   * Schedules level events. You can reuse the levels {@code Collection} but you must construct new
   * {@code PLevel}s each time you call this method.
   *
   * @param device source device
   * @param deviceTime time of even creation on device
   * @param levels level values
   * @param levelsOnScreen {@code true} if the level movement values are on the screen coordinate
   *     system, {@code false} otherwise
   * @return {@code true} if an event was scheduled, {@code false} false if it was filter out
   */
  public boolean scheduleLevelEvent(
      PenDevice device, long deviceTime, Collection<PLevel> levels, boolean levelsOnScreen) {
  ...
  public <T extends PenProvider> T getProvider(Class<T> providerClass) {
    for (PenProvider.Constructor constructor : getProviderConstructors()) {
      ...
      if (providerClass.isInstance(penProvider)) return providerClass.cast(penProvider);
    }
    return null;
  }
}
