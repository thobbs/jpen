...
public class PenState implements java.io.Serializable {
  ...
  public static class Levels implements java.io.Serializable {
    ...
    private final float[] values =
        new float
            [PLevel.Type.VALUES
                .size()]; // CUSTOM type does not store value but VALUES does not have the CUSTOM
    ...
    public void setValues(PenState.Levels levels) {
      for (int i = levels.values.length; --i >= 0; ) values[i] = levels.values[i];
      extTypeNumberToValue.clear();
      extTypeNumberToValue.putAll(levels.extTypeNumberToValue);
      for (int i = levels.values.length;
          i < values.length;
          i++) { // If a new PLevel.Type is added then transform the ext to this newer type (library
        // backwards compat.)
    ...
    public final void setValues(PLevel[] levels) {
      for (int i = levels.length; --i >= 0; ) setValue(levels[i]);
    }
    ...
    public final void setValue(int levelTypeNumber, float value) {
      if (levelTypeNumber < values.length) values[levelTypeNumber] = value;
      else setExtValue(levelTypeNumber, value);
    }
    ...
    public float getValue(int levelTypeNumber) {
      return levelTypeNumber < values.length
          ? values[levelTypeNumber]
          : getExtValue(levelTypeNumber);
    }
  ...
  private final int[] buttonValues =
      new int[PButton.Type.VALUES.size()]; // CUSTOM type does not store value.
  ...
  public boolean getButtonValue(int buttonTypeNumber) {
    return (buttonTypeNumber >= buttonValues.length
            ? getExtButtonValue(buttonTypeNumber)
  ...
  boolean setButtonValue(int buttonTypeNumber, boolean value) {
    ...
    if (buttonTypeNumber >= buttonValues.length) setExtButtonValue(buttonTypeNumber, value);
    else {
      if (value) {
        if (buttonValues[buttonTypeNumber] == 0) pressedButtonsCount++;
        buttonValues[buttonTypeNumber]++;
      } else {
        if (buttonValues[buttonTypeNumber] > 0) pressedButtonsCount--;
  ...
  private void setExtButtonValue(int buttonTypeNumber, boolean value) {
    ...
    if (value) {
      if (currentValue == 0) pressedButtonsCount++;
  ...
  void setValues(PenState penState) {
    ...
    for (int i = buttonValues.length; --i >= 0; ) buttonValues[i] = penState.buttonValues[i];
