...
public abstract class TypedClass<T extends Enum<T>> implements java.io.Serializable {
  public static final long serialVersionUID = 1l;
  public final int typeNumber;

  TypedClass(int typeNumber) {
    if (typeNumber < 0) throw new IllegalArgumentException();
  ...
  static final <T extends Enum<T>> List<T> createStandardTypes(List<T> allTypes) {
    ...
    for (int i = 0; i < customTypeOrdinal; i++) stdTypes.add(allTypes.get(i));
