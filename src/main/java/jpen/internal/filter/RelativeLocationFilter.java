...
public final class RelativeLocationFilter {
  ...
  public enum State {
    ...
    /**
     * The filter tries to see if the values are absolute or relative and sets the state if
     * possible.
     */
    UNDEFINED,
    ...
    ABSOLUTE,
    /**
     * The filter considers the sample values to be in relative mode and replace them with the
     * reference (mouse pointer info).
     */
    RELATIVE,
    /**
     * The filter does nothing. This state is used when there is not mouse pointer info available on
     * the system.
     */
    OFF;
  }
  ...
  static class SamplePoint implements Cloneable {
      ...
      for (PLevel level : sample) {
        switch (level.getType()) {
          case X:
            valuesCount++;
            levelX = level;
            if (levelY != null) break out;
            break;
          case Y:
            valuesCount++;
            levelY = level;
            if (levelX != null) break out;
            break;
        }
      }
    ...
    private void set(float x, float y, Collection<PLevel> sample) {
      if (levelX != null) levelX.value = x;
      else if (sample != null) sample.add(new PLevel(PLevel.Type.X, x));
      if (levelY != null) levelY.value = y;
      else if (sample != null) sample.add(new PLevel(PLevel.Type.Y, y));
    }
  ...
  private final Rule[] rules =
      new Rule[] {
        // new LogToFileRule(),
        new AbsoluteLocationRule(), new AbsoluteOnARowRule(), new RelativeOnSlopesRule(),
      };
   ...
   * @param sample level values to be filtered/changed according to the {@code state} of this {@code
   *     RelativeLocationFilter}
   * @param levelsOnScreen {@code true} if the given {@code sample} levels are on the screen
   *     coordinate system
   * @return {@code true} if the state of this {@code RelativeLocationFilter} changed to a
   *     definitive value.
   */
  public boolean filter(
      PenState penState, PenDevice penDevice, Collection<PLevel> sample, boolean levelsOnScreen) {
    if (!levelsOnScreen) // only levelsOnScreen are supported
    return false;
    if (state.equals(State.OFF)) return false;
    if (this.penDevice != penDevice) {
      this.penDevice = penDevice;
      state = State.UNDEFINED;
      resetRules();
    }
    if (state.equals(State.ABSOLUTE)) return false;
    if (evalIsSystemMouseDevice(penDevice)) // the system device is trusted... no need to filter.
    return false;
    if (!samplePoint.reset(sample)) return false;
    if (!setupReference()) return true;
    ...
    switch (state) {
      ...
      case UNDEFINED:
        samplePoint.set(
            penState.getLevelValue(PLevel.Type.X),
            penState.getLevelValue(
                PLevel.Type.Y)); // then it won't cause a level event because of movement
        break;
      default:
    }
    return stateChanged;
  ...
  private final PrivilegedAction<PointerInfo> getPointerInfoAction =
