...
final class XinputDevice extends AbstractPenDevice {
  ...
  XinputDevice(XinputProvider xinputProvider, XiDevice xiDevice) {
    ...
    thread =
        new Thread("jpen-XinputDevice-" + getName()) {
          @Override
          public void run() {
            while (true) {
  ...
  synchronized void setIsListening(boolean isListening) {
    ...
    xiDevice.stopWaitingNextEvent(); // xiDevice.waitNextEvent has the xiDevice sync lock.
    // stopWaitingNextEvent force xiDevice.waitNextEvent to return and
    // release the xiDevice lock.
  ...
  private int getDefaultKindTypeNumber() {
    if (isPad) return PKind.Type.IGNORE.ordinal();
    String lowerCaseName = getName().toLowerCase();
    if (lowerCaseName.contains("eraser")) return PKind.Type.ERASER.ordinal();
    if (lowerCaseName.contains("cursor")) return PKind.Type.CURSOR.ordinal();
  ...
  private void scheduleScrollEvent(int number) {
    getPenManager()
        .scheduleScrollEvent(
            this,
            xiDevice.getLastEventTime(),
            new PScroll(number == 5 ? PScroll.Type.DOWN.ordinal() : PScroll.Type.UP.ordinal(), 1));
  }
  ...
  private final float getMultRangedValue(PLevel.Type levelType) {
    ...
    // nicarran: TODO: ignore rotation or SIDE_PRESSURE depending on the name of the device? wait
    // feedback

    if (levelType.isTilt()) return devValue * RADS_PER_DEG;

    devValue = levelRanges[levelType.ordinal()].getRangedValue(devValue);

    if (isRotation) return devValue * PI_2;

    if (levelType.isMovement())
      devValue =
          xinputProvider.screenBounds.getLevelRangeOffset(levelType)
