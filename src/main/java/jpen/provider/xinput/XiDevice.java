...
final class XiDevice {
  ...
  public void setIsListening(boolean isListening) {
    synchronized (xiBus) {
      synchronized (XiBus.macrofLock) {
        int attempts = 0;
        while (true) {
          // TODO: change setIsListening to return true on success to avoid doing a
          // getIsListening... this requires a nativeVersion change.
  ...
  public Range getLevelRange(PLevel.Type levelType) {
    synchronized (xiBus) {
      ...
      return new Range(
          getLevelRangeMin(cellIndex, typeIndex), getLevelRangeMax(cellIndex, typeIndex));
      // }
    }
  }
  ...
  public EventType getLastEventType() {
    synchronized (xiBus) {
      ...
      if (lastEventTypeOrdinal < 0) return null;
