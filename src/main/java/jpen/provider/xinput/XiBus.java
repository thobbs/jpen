...
final class XiBus {
  /**
   * To be thread-safe against the c/utils/macro.c-h framework we synchronize against {@code
   * macrofLock} (private lock). To be thread-safe against the X-Server we synchronize against the
   * XiBus instance (public lock):
   *
   * <ul>
   *   <li>The X server is not thread safe, requests must be made one at a time (per connection).
   *       Each XiBus instance holds one X server connection.
   *   <li>The c/utils/macro.c-h framework is not thread safe, its functions must be serialized (per
   *       class-row).
   * </ul>
   *
   * Sometimes is needed to synchronize against the X-Server but not against the macro.c/f framework
   * (1), sometimes is needed to synchronize against the macro.c/f framework alone (2), and
   * sometimes is needed to synchronize against both (3). The locks must be acquired in this order:
   * XiBus instance first (public lock), {@code macrofLock} later. Trying to hold XiBus instance
   * lock after holding macrofLock will cause dead-locks.
   */
  ...
  /**
   * This method does not use functions of the macro.c-h framework and does not use the X server
   * connection, then it is safe to call outside the locks.
   */
  ...
  public synchronized void setXiDevice(int xiDeviceIndex) throws Exception {
    synchronized (macrofLock) {
      ...
      if (xiDeviceCellIndex < 0) throw new Exception(getError());
  ...
  public synchronized void refreshXiDeviceInfo() {
    synchronized (macrofLock) {
      if (refreshDeviceInfo(cellIndex) != 0) throw new IllegalStateException(getError());
    }
  }
