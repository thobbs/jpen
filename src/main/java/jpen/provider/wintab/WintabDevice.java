...
class WintabDevice extends AbstractPenDevice {
  ...
  private static final boolean PRINT_ALTITUDE_AND_AZIMUTH =
      Boolean.valueOf(System.getProperty(PRINT_ALTITUDE_AND_AZIMUTH_SYSTEM_PROPERTY));
  ...
  private int getDefaultKindTypeNumber() {
    ...
    switch (cursorType) {
      ...
      case UNDEF:
        ...
        if (deviceName.contains("stylus")) return PKind.Type.STYLUS.ordinal();
        if (deviceName.contains("eraser")) return PKind.Type.ERASER.ordinal();
  ...
  */
  ...
  private void scheduleLevelEvent() {
    ...
    getPenManager()
        .scheduleLevelEvent(this, wintabProvider.wintabAccess.getTime(), changedLevels, true);
    changedLevels.clear();
  }
  ...
  private float getMultRangedValue(PLevel.Type type) {
    if (PLevel.Type.TILT_TYPES.contains(type)) {
      ...
      if (altitude < 0) altitude = -altitude; // when using the eraser the altitude is upside down.
      double rangedAltitude =
          altitude
              / wintabProvider.getLevelRange(PLevel.Type.TILT_Y)
                  .max; // 0 (0deg) <= rangedAltitude <= 1 (90deg)
      if (rangedAltitude == 1) { // optimization
        if (PRINT_ALTITUDE_AND_AZIMUTH) System.out.println("rangedAlt: 1");
        return 0;
      }
      double betha =
          rangedAltitude
              * PI_over_2; // see tiltOnWintab.xoj to understand the meaning of betha and theta

      double azimuth = wintabProvider.wintabAccess.getValue(PLevel.Type.TILT_X);
      double rangedAzimuth =
          azimuth / getCorrectAzimuthRangeMax(); // 0 (0deg) <= rangedAzimuth < 1 (360deg)
      double theta = rangedAzimuth * PI_2 - PI_over_2;

      if (PRINT_ALTITUDE_AND_AZIMUTH)
        System.out.printf(
            "alt: %4.0f - az: %4.0f | rangedAlt: %1.3f - rangedAz: %1.3f \n",
    ...
    float rangedValue =
        wintabProvider
            .getLevelRange(type)
            .getRangedValue(wintabProvider.wintabAccess.getValue(type));

    if (PLevel.Type.MOVEMENT_TYPES.contains(type)) {
      if (type.equals(PLevel.Type.Y)) rangedValue = 1f - rangedValue;
      rangedValue =
          wintabProvider.screenBounds.getLevelRangeOffset(type)
  ...
  /**
   * The maximum value for the azimuth must be corrected to be the equivalent of 360deg. On Wacom
   * Intuos the values is the corresponding Range.max but on other tablet it is assumed that the
   * value could less than 360 (1 less than 360 given its resolution). So we simply add to the
   * Range.max until we get a multiple of 360. See bug
   * https://sourceforge.net/tracker/?func=detail&aid=3603044&group_id=209997&atid=1011964 .
   */
  private float getCorrectAzimuthRangeMax() {
    if (correctAzimuthRangeMax != -1) return correctAzimuthRangeMax;
    correctAzimuthRangeMax = wintabProvider.getLevelRange(PLevel.Type.TILT_X).max;
    while (correctAzimuthRangeMax % 360 != 0) correctAzimuthRangeMax++;
