...
final class KeyboardDevice extends AbstractPenDevice {
  ...
  class AwtListener implements AWTEventListener {
    ...
    private void fireModifiers() {
      ...
      ModifiersInfo modifiersInfo =
          lastModifiersInfo; // local copy because this method is concurrent
      if (modifiersInfo == null) return;
      for (PButton.Type modifierType : PButton.TypeGroup.MODIFIER.getTypes()) {
        ...
        getPenManager()
            .scheduleButtonEvent(
                KeyboardDevice.this, modifiersInfo.when, new PButton(modifierType, value));
      }
    }
  }
  ...
  void setPaused(boolean paused) {
    if (!paused) awtListener.fireModifiers();
  }
}
