...
final class CocoaAccess {
  ...
  public static final int WACOM_CAPABILITY_TILTMASK =
      WACOM_CAPABILITY_TILTXMASK | WACOM_CAPABILITY_TILTYMASK;
  ...
  protected void postProximityEvent(
    ...
    if (vendorID == WACOM_VENDOR_ID) {
      ...
      // vendorPointingDeviceType seems to always be zero (even if I print out the value in
      // Objective-C)
  ...
  private void scheduleButtonEvent(long deviceTime, PButton.Type type, boolean pressed) {
    cocoaProvider
        .getPenManager()
        .scheduleButtonEvent(device, deviceTime, new PButton(type.ordinal(), pressed));
  }
}
