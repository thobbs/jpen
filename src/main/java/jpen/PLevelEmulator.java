...
public final class PLevelEmulator {
  ...
  private final List<List<ButtonTriggerPolicy>> kindTypeToButtonTypeToButtonTriggerPolicy =
      new ArrayList<List<ButtonTriggerPolicy>>();
  private final List<ButtonTriggerPolicy> activeButtonTriggerPolicies =
      new ArrayList<ButtonTriggerPolicy>();
  private final BitSet activeLevelTypes = new BitSet();
  private final List<ButtonTriggerPolicy> kindTypeToAlwaysActiveButtonTriggerPolicy =
      new ArrayList<ButtonTriggerPolicy>();

  public void setPressureTriggerForLeftCursorButton(float pressure) {
    setTriggerForLeftCursorButton(
        new ButtonTriggerPolicy(PLevel.Type.PRESSURE.ordinal(), pressure, 0f));
  }
  ...
  public void setTrigger(
      PKind.Type kindType, PButton.Type buttonType, ButtonTriggerPolicy triggerPolicy) {
    setTrigger(kindType.ordinal(), buttonType.ordinal(), triggerPolicy);
  }

  public synchronized void setTrigger(
      int kindType, int buttonType, ButtonTriggerPolicy triggerPolicy) {
    setTrigger(kindType, buttonType, triggerPolicy, false);
  }

  public synchronized void setTrigger(
      int kindType, int buttonType, ButtonTriggerPolicy triggerPolicy, boolean alwaysActiveOnKind) {
    List<ButtonTriggerPolicy> buttonTypeToButtonTriggerPolicy =
        getButtonTypeToButtonTriggerPolicy(kindType);
  ...
  private List<ButtonTriggerPolicy> getButtonTypeToButtonTriggerPolicy(int kindType) {
    ...
    List<ButtonTriggerPolicy> buttonTypeToButtonTriggerPolicy =
        kindTypeToButtonTypeToButtonTriggerPolicy.get(kindType);
    if (buttonTypeToButtonTriggerPolicy == null) {
      kindTypeToButtonTypeToButtonTriggerPolicy.set(
          kindType, buttonTypeToButtonTriggerPolicy = new ArrayList<ButtonTriggerPolicy>());
    }
  ...
  void scheduleEmulatedEvent(PButtonEvent buttonEvent) {
    ...
    if (buttonEvent.button.value) emulatedLevel = emulateOnPress(buttonEvent.button.typeNumber);
    else emulatedLevel = emulateOnRelease(buttonEvent.button.typeNumber);
    if (emulatedLevel != null) {
      pen.penManager.scheduleLevelEvent(
          pen.penManager.emulationDevice, buttonEvent.time, Collections.singleton(emulatedLevel));
    }
  }
  ...
  private boolean onAlwaysActivePolicy(int kindType, int levelType) {
    ensureListSize(kindTypeToAlwaysActiveButtonTriggerPolicy, kindType);
    ButtonTriggerPolicy alwaysActivePolicy =
        kindTypeToAlwaysActiveButtonTriggerPolicy.get(kindType);
