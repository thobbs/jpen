...
public class Pen extends PenState {
  ...
  private final class MyThread extends Thread {
    ...
    final class Waiter extends Object {

      boolean waitForNewEvent() throws InterruptedException {
        if (lastDispatchedEvent.next != null) return false;
        synchronized (this) {
          if (lastDispatchedEvent.next != null) return false;
          if (!stopRunning) wait(0);
          return true;
        }
      }
    ...
    private final Runnable penTockFirer =
        new Runnable() {
          // @Override
          public void run() {
    ...
    private void firePenTock() throws InterruptedException, InvocationTargetException {
      if (getListenersArray().length == 0) return;
      if (firePenTockOnSwing) SwingUtilities.invokeAndWait(penTockFirer);
      else penTockFirer.run();
    }
  ...
  /**
   * @param firePenTockOnSwing If {@code true} then {@link PenListener#penTock(long)} is called from
   *     the event dispatch thread. {@code false} by default.
   */
  ...
  /**
   * Changes the event firing frequency. The pen collects device (tablet) data points and stores
   * them in a buffer. The data points are taken from this buffer and fired as {@link PenEvent}s at
   * this frequency.
   *
   * <p>This method returns immediately, the change of frequency will happen after all the pending
   * buffered events are processed.
   *
   * @param frequency event firing frequency. If frequency is greater than {@code 1000} then pen
   *     events will be fired as soon as possible (no buffering).
