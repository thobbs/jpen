...
public abstract class AbstractPenOwner implements PenOwner {
  ...
  /**
   * Force stopping a drag-out operation if it was in progress. This method must be called when
   * entering the {@link PenClip} to stop the drag-out operation if it was still in progress.
   *
   * <p>This method must be called while holding the {@link
   * jpen.owner.PenOwner.PenManagerHandle#getPenSchedulerLock()} (See {@link
  ...
  /**
   * Starts a drag-out operation if there are pressed buttons and installs a penListener which
   * automatically stops the drag-out operation when all the buttons are unpressed.
   *
   * <p>This method must be called while holding the {@link
   * jpen.owner.PenOwner.PenManagerHandle#getPenSchedulerLock()} (See {@link
   * PenOwner#isDraggingOut()}).
  ...
  protected final boolean startDraggingOut() {
    ...
    if (!getPen().hasPressedButtons()) return false;
    isDraggingOut = true;
  ...
  public final void setPenManagerHandle(PenManagerHandle penManagerHandle) {
    if (this.penManagerHandle != null)
      throw new IllegalStateException(
          "The penManagerHandle has already being set. This PenOwner can't be used on multiple PenManagers.");
    this.penManagerHandle = penManagerHandle;
    init();
  }

  /**
   * Override this method to do initialization. This method is called after the {@link
   * #penManagerHandle} is set.
   */
