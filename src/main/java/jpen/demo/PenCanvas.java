...
class PenCanvas extends JComponent {
    ...
    AwtPenToolkit.addPenListener(
        ...
        new PenAdapter() {
          ...
          public void penTock(long availableTime) {
            // System.out.println("tock="+System.currentTimeMillis()+", available
            // time="+availableTime);
          ...
          public void keyPressed(KeyEvent ev) {
            if (ev.isControlDown() && ev.getKeyCode() == KeyEvent.VK_D)
              JOptionPane.showMessageDialog(
                  null,
                  "<html>Testing dialog window:<p> the pen buttons must be disabled when this dialog appears.</html>");
          }
        });
    addMouseListener(
        new MouseAdapter() {
  ...
  private void addCursorAreaToDirtyArea() {
    ...
    if (isDirty) dirtyArea.add(rectangle);
    else dirtyArea.setRect(rectangle);
    isDirty = true;
  }
