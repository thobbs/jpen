...
public class StatusReport {
  ...
  private void appendLine(String line, int indent) {
    line = line.trim();
    for (int i = indent; --i >= 0; ) report.append("   ");
    report.append(line);
    if (!line.endsWith("\n")) report.append("\n");
  }
  ...
  private void appendPenThreadCrashInfo(PenManager penManager) {
    ...
    if (penThreadCrashException != null) {
      appendLine(
          "Pen Thread Crashed: " + ThrowableUtils.evalStackTraceString(penThreadCrashException));
    }
  }
  ...
  private static final Set<String> PRIVATE_SYSTEM_PROPERTIES =
      new HashSet<String>(
          Arrays.asList(
              new String[] {
                "user.dir", "java.io.tmpdir", "line.separator", "user.home", "user.name",
              }));
  ...
  private void appendProvidersInfo(PenManager penManager) {
    ...
    for (PenProvider.Constructor constructor : penManager.getProviderConstructors()) {
      ...
      PenProvider.ConstructionException constructionException =
          constructor.getConstructionException();
      String constructionExceptionStackTrace = "none";
      if (constructionException != null) {
        constructionExceptionStackTrace =
            ThrowableUtils.evalStackTraceString(constructionException);
      }
      ...
      if (penProvider != null) {
        ...
        if (nativeVersion != -1) {
          appendLine(
              "Native Version-Build(Expected): "
                  + constructor.getNativeVersion()
                  + "-"
                  + constructor.getNativeBuild()
                  + "("
                  + constructor.getExpectedNativeBuild()
                  + ")",
              2);
        }
